...
onMounted(async () => {
  ...
  // on the next painted frame. The window starts hidden (StartHidden), so the
  ...
  // fire and the window would stay invisible forever.
  await nextTick();
  WindowShow();
});

// quitApp flushes the pending session autosave BEFORE quitting, so the last
// keystrokes are never lost to the debounce window. Dirty tabs need no confirm
// dialog: they are part of the flushed session and come back on the next launch.
...
// Double-clicking the title bar maximizes/restores the window (the standard
// Windows gesture). The bar is the window's drag region, and WKWebView on macOS
// swallows click/dblclick there (the mousedown starts a window drag), so we
// detect the double-click from two quick mousedowns instead. The window
...
function onTitlebarMousedown(e: MouseEvent) {
  ...
  if (e.timeStamp - lastTitleDown < 500) {
    lastTitleDown = 0;
    WindowToggleMaximise();
  } else {
    lastTitleDown = e.timeStamp;
  }
}
</script>
